...
function TweaksPanel({ title = 'Настройки', children }) {
  const [open, setOpen] = React.useState(false);
  const dragRef = React.useRef(null);
  const offsetRef = React.useRef({ x: 16, y: 16 });

  // Expose open/close globally so the gear button in sidebar can trigger it
  React.useEffect(() => {
    window.__openSettings = setOpen;
  ...
  // Open with keyboard shortcut ⌘, (comma)
  React.useEffect(() => {
    const onKey = (e) => {
...
function TweakColor({ label, value, options, onChange }) {
  if (!options || !options.length) {
    return (
      <div className="twk-row twk-row-h">
        <div className="twk-lbl"><span>{label}</span></div>
        <input type="color" style={{ width: 56, height: 22, border: 'none', borderRadius: 6, padding: 0, cursor: 'default' }}
...
function TweakButton({ label, onClick, danger = false }) {
  ...
  return (
    <button type="button" onClick={handleClick} style={{
      appearance: 'none', height: 28, padding: '0 12px', border: 0, borderRadius: 7,
      background: confirming ? 'rgba(220,38,38,.12)' : danger ? 'rgba(0,0,0,.06)' : 'rgba(0,0,0,.78)',
      color: danger ? (confirming ? '#dc2626' : 'rgba(41,38,27,.7)') : '#fff',
      font: 'inherit', fontWeight: 500, cursor: 'default', width: '100%', textAlign: 'left',
      transition: 'background .15s, color .15s',
    }}>
      {confirming ? '⚠ Нажмите ещё раз для подтверждения' : label}
    </button>
  );
}
...
Object.assign(window, {
  ...
  TweakSlider, TweakToggle, TweakRadio, TweakSelect, TweakColor, TweakButton,
});
